...
public class SetTest {
	 ...
	 */
		...
		//set에 중복되는 데이터를 추가 하면 false를 반환하고 데이터는 추가되지 않는다.
		...
		//Set의 데이터를 수정하려면 수정하는 명령이 따로 없기 때문에 해당자료를 삭제하고 
		//새로 추가하는 방법을 사용한다.
...
//		System.out.println("set 데이터 ==>"+ hs1);
		...
		/*
		 * Set의 데이터는 순서(index)가 없기 때문에 List처럼 index로 데이터를 하나씩
		 * 불러올수 없다. 그래서 데이터를 하나씩 얻기 위해서는 Iterator형 
		 * 객체로 변환해야 한다.
		...
		//			다으번째에 데이터가 있으면 true, 없으면 false를 반환하고 메서드가 끝난다.
		while(it.hasNext()) {
			//Iterator의 next()메서드
			//		==> Iterator의 포인터를 다음번째 위치로 이동시키고 이동한 곳의 데이터를 반환한다.
...
//		int a = rnd.nextInt(24) + 1;
		
		HashSet<Integer> testSet = new HashSet<Integer>();
		Random rnd = new Random();
		while(testSet.size() < 3) {
			testSet.add( rnd.nextInt(24) + 1  );
		}
		System.out.println("당첨자 번호: "+ testSet);
		
		// Set형의 자료를 List형으로 변환하기
		//List객체를 생성할때 생성자의 인수값으로 Set데이터를 넣어준다
