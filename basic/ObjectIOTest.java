...
//			oos.flush();	// 남아있는 객체를 모두 옮기기. 더 구현해야 할 기능이 있다면 마지막에 close를 사용하고 없다면 바로 아래 flush를 쓴다. 
			oos.close();  // 스트림 닫기. flush의 기능도 같이 해준다.
			
			//---------------------------
			
			// 저장된 객체를 읽어와 그 내용을 화면에 출력하기
			// 입력용 스트림 객체 생성
			ObjectInputStream ois = new ObjectInputStream(new BufferedInputStream(new FileInputStream("d:/d_other/memObj.dat")));
			try {
				System.out.println("객체 읽기 작업 시작...");
				Object obj ;  // 읽어온 데이터가 저장될 변수
				
				// readObject()메서드는 데이터를 끝까지 다 읽어오면
				//	EOFException이 발생한다.
