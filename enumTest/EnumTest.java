...
/*
  - enum(열거형) ==> 서로 관련있는 상수들의 집합을 나타낸다.
  			   ...
  			   ==> 열거형은 클래스처럼 독립된 java파일에 만들수 있고,
  			   		하나의 java파일에 다른 클래스와 같이 만들수 있고, (class test == enum test)
  			   		클래스 안에 내부 클래스처럼 만들 수 있다. 
  			   		
  -	열거형의 속성 및 메서드
  	 1. name() ==> 열거형 상수의 이름을 문자열로 반환한다. (=> RED, GREEN, BLUE, ONE, TWO, THREE)
  ...
  방법2)
  	 enum 열거형이름 {값들의 갯수와 종료는 같아야한다.
  	 		상수명1(값들...), 상수명2(값들...), 상수명2(값들...);
  	 		// '값들'을 저장할 변수들 선언
  	 		  private 자료형이름 변수명1;
  	 		  ...
  	 		// 열거형의 생성자를 만든다.
  	 		// 열거형의 생성자는 '열거형상수'에 설정된 '값들'을 변수에 초기화하는 약할을 수행한다.
  	 		...
  	 		// 구성된 '값들'을 외부에서 불러올 수 있도록 하기위해서 getter메서드를 작성한다.
  	 }
...
public class EnumTest {
	...
	public static void main(String[] args) {
		...
		Color mycol1 = Color.valueOf("GREEN"); //3번
		Color mycol2 = Color.GREEN; //4번
		...
		// switch문 비교
		// switch문의 case문에 상수를 지정할 때는 열거형이름은 생략하고 상수이름만 기술해야함.
		switch(mycnt) {
